...
class Tls13Client {
    ...
    public function post_json($path, $body, $token=null, $link_id=null) {
        list($status, $headers, $text) = $this->request("POST", $path, $body, $token, $link_id);
...
echo "Final URLs after TagHelper.urlTagReplace:\n";
foreach ($final_urls as $index => $url) {
    echo "[$index] $url\n";
}

if (!empty($final_urls)) {
    $url_index = isset($options["url-index"]) ? (int)$options["url-index"] : 0;
    $selected_index = min(max($url_index, 0), count($final_urls) - 1);
    echo "Selected final URL [$selected_index]:\n";
    echo $final_urls[$selected_index] . "\n";
}

if (isset($options["json-out"])) {
    $out = [
        "device_uuid" => $device_uuid,
        "adid" => $adid,
        "channel" => $channel,
        "get_channel_url_request_headers" => $client->last_request_headers,
        "get_channel_url_request_body" => $request_body,
        "get_channel_url_response" => $payload,
        "raw_urls" => $raw_urls,
        "final_urls" => $final_urls
    ];
    file_put_contents($options["json-out"], json_encode($out, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
    echo "JSON written: {$options["json-out"]}\n";
}
