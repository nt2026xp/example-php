...
class CKeyManager
{
    ...
    private function sendHttpRequest($params)
    {
        ...
        // PHP 8.5+ curl_close已废弃，但为了兼容保留，加了@抑制警告
        @curl_close($ch);
...
$cacheTimeoutLive = 80; // 直播缓存超时 80秒
...
for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
    ...
    $context = stream_context_create([
        'http' => [
            'timeout' => 10,
            'header' => "User-Agent: qqlive\r\n"
        ]
    ]);
    $m3u8Content = @file_get_contents($playUrl, false, $context);
...
// 输出M3U8内容
$m3u8Content = preg_replace("/(.*?.ts)/i", $baseUrl . "$1", $m3u8Content);
echo trim($m3u8Content);
